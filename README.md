...
python3 grab_odcr.py --per-type g7.48xlarge:1 --region us-west-2 --live --watch   # 抢别的区域（机型是区域级的！）
python3 grab_odcr.py --cancel-all --live --region us-west-2   # 释放也要带同一个 region
python3 grab_odcr.py --cancel-all --live             # 释放全部、停止计费（默认 us-east-1）
```
...
| `--region R` | 区域，默认 `us-east-1`。⚠️ **机型是区域级的**：同一个机型名不是每个区域都有（如 `g7.48xlarge` 在 us-west-2 有、us-east-1 没有）。目标区域没有的机型会被丢弃并 warning「could not resolve」——遇到这个先想想是不是 region 没对，而不是机型名打错。**每个 region 独立管理**：`--list` / `--cancel-all` 都要带同一个 `--region`，默认区释放了不等于别的区也释放了 |
