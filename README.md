...
旧版脚本抢的 count=1 碎片、或手动建的预留还在时：新容量只会挑其中一个持续 +1，**其余原样保留、不动也不合并**——合并要先 cancel 再加量，中间容量会回公共池被人抢走，大促期间绝不做。进度统计按全部对象的台数加总，不重抢不少算。想收敛到一个对象：等大促结束 `--cancel-all` 清零后重抢。
...
| 参数 | 作用 |
|------|------|
| `--per-type TYPE:COUNT ...` | **要抢什么、各抢几台**（唯一的抢占入口）。COUNT 默认是**绝对持有目标**：已持有 ≥ COUNT 就不再抢（这是重启安全的根基）。每种机型一个独立目标，各抢各的、互不拖累。COUNT 是台数、正整数；重复机型以最后一个为准；格式错直接退出（exit 2）。任何 EC2 机型都行，启动时自动向 AWS 验证，不认识的丢弃并 warning |
...
| `--azs ...` | 锁定 AZ，如 `--azs us-east-1b us-east-1d`。COUNT 是**在这些 AZ 里凑够的总台数**，哪个 AZ 有货抢哪个，不做 per-AZ 均分。不传 = region 全部 AZ |
