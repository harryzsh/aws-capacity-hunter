...
| 参数 | 作用 |
|------|------|
| `--per-type TYPE:COUNT ...` | **要抢什么、各抢几台**（唯一的抢占入口）。每种机型一个独立台数目标，各抢各的、互不拖累。COUNT 是台数、正整数；重复机型以最后一个为准；格式错直接退出（exit 2）。任何 EC2 机型都行，启动时自动向 AWS 验证，不认识的丢弃并 warning |
...
| `--end-hours N` | 计费保险：预留对象 N 小时后自动过期 |
